...
from ..models.models import Todos,TodoRequest
...
async def render_edit_todo(request:Request,id:int,db:db_dependency):
  try:
    user = await get_current_user(request.cookies.get('access_token'))
    if user is None:
      return redirect_to_login()
...
async def read_todo(user:user_dependency,db:db_dependency,id:int = Path(gt=0)):
  ...
  todo_model = db.query(Todos).filter(Todos.id==id,Todos.owner_id==user.get('id')).first()
...
async def delete_todo(user:user_dependency,db:db_dependency,id:int=Path(gt=0)):
  ...
  if todo_model is None:
    raise HTTPException(status_code=404,detail='Todo not found desde el model is none.')
